...
// A Mock contains a Request to compare to *http.Requests and the
// http.Response and error value to return if it matches.
type Mock struct {
...
// ReadJSON reads a Set (in JSON format) from the io.Reader r and returns it
// (or the resultant error). The JSON should be an array of objects, whose
// keys correspond to a Mock (where In is a Request, Out is a Response, and
// Err is an error). The In strings will be compiled to regular expressions,
// and the Out strings (and Err string) will be treated as Go templates. Names
// submatches in the In strings may be used as variables in the Out and Err
// templates.
//   Note:
