...
// A Match is a map of replacement strings for Response templates, which have
// a number of methods that allow them to be built up from Regexps.
type Match map[string]string

// doString adds to m submatches found when matching s to r; returns false if
// s doesn't match r.
...
// doStrings applies doString to all of the strings, and returns true if any
// of them match (strings after the first matching string are ignored).
